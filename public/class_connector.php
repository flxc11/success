...
class connector {
	...
	function systemfile($trueclass = false) {
		...
		if (!empty($CONFIG['cer_key'])) {
			...
			$xmlfile = $getnetval . 'index.php?ac=siteauthentication&at=volcheck&siteurl=' . urlencode(admin_ClassURL) . '&vol=' . db_release . '&dbcode=' . db_pscode . '&db_keycode=' . db_keycode;
	...
	function sendsms($smsContent = null, $toMoblie = 0) {
		if (!$this->CON['is_moblie']) return false;
		...
		$data = array('ac' => 'public', 'at' => 'smssend', 'userid' => $userid, 'smssnid' => $smssnid, 'smskey' => md5($smskey), 'smsdomain' => $smsdomain, 'smscontent' => $smsContent, 'smsmoblie' => $toMoblie, 'ipadd' => $ipadd, 'moblienumber' => $moblienumber);
